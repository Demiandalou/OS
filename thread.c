...
bool
less_tick(const struct list_elem *a,const struct list_elem *b,void *aux){/* true if a's sleepTime<b's */
  struct thread *athread=list_entry(a,struct thread,elem);
  struct thread *bthread=list_entry(b,struct thread,elem);
  return (athread->tsleep_time<bthread->tsleep_time);
}

bool
less_priority(struct list_elem *A,struct list_elem *B){/* true if a's sleepTime<b's */
